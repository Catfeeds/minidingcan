...
class ProductController extends PublicController{
	...
	*/
	...
	public function set_wm(){
		$pro_id = intval($_REQUEST['pro_id']);
		$tj_update=M('product')->field('shop_id,is_wm')->where('id='.intval($pro_id).' AND del=0')->find();
		if (!$tj_update) {
			$this->error('菜单不存在或已下架删除！');
			exit();
		}

		//查推荐type
		$data = array();
		$data['is_wm'] = $tj_update['is_wm']==1 ? 0 : 1;
		$up = M('product')->where('id='.intval($pro_id))->save($data);
		if ($up) {
			$this->redirect('index',array('page'=>intval($_REQUEST['page'])));
			exit();
