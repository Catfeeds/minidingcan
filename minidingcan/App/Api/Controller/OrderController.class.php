...
class OrderController extends PublicController {
	...
	public function index(){
        ...
        $order = $orders->where($condition)->order('id desc')->field('id,order_sn,pay_sn,status,price,type,product_num,order_type')->limit(7)->select();
		foreach ($order as $n=>$v) {
			...
			$order[$n]['pro_count'] = $orderp->where('order_id='.intval($v['id']))->getField('SUM(num)');
		}
	...
	public function get_more(){
        ...
        $order = $orders->where($condition)->order('id desc')->field('id,order_sn,pay_sn,status,price,type,product_num,order_type')->limit($limit.',7')->select();
		foreach ($order as $n=>$v){
			$order[$n]['desc'] = $order_status[$v['status']];
			$prolist = $orderp->where('order_id='.intval($v['id']))->find();
			$order[$n]['photo_x'] = __DATAURL__.$prolist['photo_x'];
	...
	public function order_details(){
		...
		$order_info = $orders->where('id='.intval($order_id).' AND del=0')->field('id,order_sn,status,addtime,amount,type,tel,receiver,address_xq,yunfei,remark')->find();
		if (!$order_info) {
		...
		//订单状态
		$order_status=array('0'=>'已取消','10'=>'待付款','20'=>'待配送','30'=>'待确认','40'=>'已完成','50'=>'已完成');
		//支付类型
		...
		foreach ($pro as $k => $v) {
			$pro[$k]['zprice'] = floatval($v['price']*$v['num']);
			$num += intval($v['num']);
		}
		$order_info['procount'] = intval($num);
